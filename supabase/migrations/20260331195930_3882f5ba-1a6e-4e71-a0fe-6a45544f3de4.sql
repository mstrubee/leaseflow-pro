DROP INDEX IF EXISTS public.unique_general_folder_contract;

ALTER TABLE public.general_folders
ADD CONSTRAINT unique_general_folder_contract UNIQUE (contract_id);
...
DECLARE
  ...
  v_counter int := 2;
BEGIN
  SELECT id INTO v_root_id FROM public.general_folders WHERE is_contract_root = true LIMIT 1;
  IF v_root_id IS NULL THEN RETURN NEW; END IF;

  SELECT COALESCE(MAX(display_order), 0) + 1 INTO v_max_order
  FROM public.general_folders
  WHERE parent_id = v_root_id;

  SELECT EXISTS(
    SELECT 1 FROM public.contract_companies cc
  ...
  IF EXISTS (
    SELECT 1 FROM public.general_folders
    WHERE parent_id = v_root_id AND name = v_folder_name
  ) THEN
    SELECT ca.street INTO v_street
    FROM public.contract_addresses ca
  ...
  WHILE EXISTS (
    ...
    WHERE parent_id = v_root_id
      ...
      AND contract_id IS DISTINCT FROM NEW.id
  ) LOOP
    v_folder_name := v_base_name || ' (' || v_counter || ')';
    v_counter := v_counter + 1;
  END LOOP;

  INSERT INTO public.general_folders (name, parent_id, display_order, contract_id)
