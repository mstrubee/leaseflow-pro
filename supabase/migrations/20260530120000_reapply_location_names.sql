-- Re-aplica los nombres reales de los locales Agroplanet/Autoplanet.
--
-- Contexto: las migraciones originales de nombres (20260529150000) quedaron sin
-- aplicar porque la cadena se bloqueó en 20260529140000 (ALTER TYPE). Lovable
-- regeneró el esquema y re-sembró los 90 locales con nombres genéricos
-- ('Agroplanet'/'Autoplanet') vía 20260529181959 (ON CONFLICT DO NOTHING).
-- Esta migración corre DESPUÉS de ese re-seed y fija los nombres definitivos.
-- Es idempotente: solo UPDATE por poi_id.

-- Garantiza la columna de minutos por form (su migración original pudo no aplicarse)
alter table public.maintenance_route_forms
  add column if not exists estimated_minutes integer default 30;

-- Nombres Agroplanet (extraídos de Plantilla Locales GP.xlsx por geocodificación inversa)
...
-- Autoplanet sin nombre en Excel (nombrados por ciudad vía geocodificación inversa)
