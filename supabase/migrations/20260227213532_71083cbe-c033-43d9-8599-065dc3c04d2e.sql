...
-- Function to get all dashboard stats aggregated server-side
...
BEGIN
  WITH contract_data AS (
    ...
    GROUP BY c.id, c.name, c.status, c.requires_special_attention, ca.region, ca.commune
  ),
  totals AS (
    SELECT
      ...
      count(*) FILTER (WHERE status = 'en_negociacion') AS total_negociacion,
...
-- Function to get termination alerts
CREATE OR REPLACE FUNCTION public.get_termination_alerts()
RETURNS jsonb
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      'id', c.id,
      'name', c.name,
      'required_exit_date', tn.required_exit_date,
      'notice_type', tn.notice_type,
      'issuer_name', tn.issuer_name
    ) ORDER BY tn.required_exit_date
  ), '[]'::jsonb)
  INTO result
  FROM contracts c
  JOIN termination_notices tn ON tn.contract_id = c.id
  WHERE c.deleted_at IS NULL
    AND tn.required_exit_date IS NOT NULL;
