...
export function CriticalAlertsDashboard({
  ...
  const { kpis, criticalDocuments } = useMemo(() => {
            ...
            <Table>
              ...
              <TableBody>
                {criticalDocuments.map((doc, idx) => (
                  <TableRow key={`${doc.contractId}-${doc.itemId}-${idx}`} className={getRowBgColor(doc)}>
                    <TableCell>
                      <div>
                    ...
                    <TableCell className="text-center">
                      <Select 
                        value={doc.status} 
                        onValueChange={(v) => onStatusChange(doc.contractId, doc.itemId, v as PatentDocStatus)}
                      >
                    ...
                    <TableCell>
                      {doc.endDate 
                        ? format(new Date(doc.endDate), 'dd/MM/yyyy', { locale: es })
                        : '-'
                      }
                    </TableCell>
                    <TableCell className="text-center">
                      {doc.daysRemaining !== undefined ? (
                        <span className={
                          ...
                          {doc.daysRemaining < 0 
                            ? `${Math.abs(doc.daysRemaining)} atraso`
                            : doc.daysRemaining
                          }
                        </span>
                      ) : '-'}
                    ...
                    <TableCell className="text-right">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => onNavigateToDocument(doc.contractId, doc.itemId)}
                      >
